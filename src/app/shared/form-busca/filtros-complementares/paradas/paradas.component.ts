...
export class ParadasComponent implements OnInit {
  ...
  opcoes: OpcoesDeParada[] = [
    { display: 'Direto', value: '0' },
    { display: '1 conexão', value: '1' },
    { display: '2 conexões', value: '2' },
    { display: 'Mais de 2 conexões', value: '3' },
  ];
  ...
  alternarParada(opcao: OpcoesDeParada, checked: boolean) {
    if (!checked) {
      this.opcoesSelecionada = null;
      this.formBuscaService.formBusca.patchValue({ conexoes: null });
      return;
    }
